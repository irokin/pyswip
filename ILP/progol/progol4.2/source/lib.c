...
LIB(l_layer) {
	...
	while((ccl=ccl_fread(in))!=(ITEM)I_TERM) {
	    ...
	    else if(CTYPE(ccl)==pdot0) {	/* Statement */
	      cl=F_ELEM(0l,ccl);
		...
		*f_ins(psym,oldbdef)=b_copy(*e1);
		...
		sprintf(mess1,"%s - Time taken %.2fs, Cum. time %.2fs",mess,
			cputime()-start, cputime()-start0);
		g_message(1l,mess1);
	      }
	    }
	    else if(CTYPE(ccl)==pquest) {	/* Headless clause */
		start=cputime();
		interp_quest(ccl);
		ccl_swrite(mess,ccl);
...
LIB(l_write) {
	/* ITEM term=p_copy(args[0l],subsin[0l],TRUE);
	ct_renumv(term); */
	noquotes=TRUE;
	/* p_fwrite(plgout,term); */
...
STRING plib[] = {
	...
	"number(X) :- float(X).",
	"otherwise.",
	"prune :- hypothesis(Head,Body,_), prune(Head,Body).",
