...
ITEM psamps,bpsamps,costs,prune;
double noiselim,clim;
LONG pprune0,pprune2,inflate;
LONG vlim,pnlim,hlim,pdot2,pempty,pminus,pplus,phash,pdiv2,errorchk;
...
PREDICATE cover,condition,conditioning,memoing,posonly,reductive,sampling,
	searching,splitting,tracing;
PREDICATE pdebug,namecmp,noquotes,reconsult,mseen;
STRING editor;


ITEM tag=(ITEM)0x180300;
...
c_open()
	...
	pprune2=QP_ston("prune",2l); /* prune/2 */
	pminus=QP_ston("-",0l);		/* -/0 */
...
c_close()
	...
	i_deletes(allpsyms,nums,spies,bpsamps,fsymtlib,(ITEM)I_TERM);
	i_deletes(commutes,repeats,labels,prune,(ITEM)I_TERM);
